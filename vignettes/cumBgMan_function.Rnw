...
% temp.init, pres.init
% pres.resid, rh.resid, rh.resid.init
...
In this example, we will use the longcombo example data set included in the biogas package: \texttt{sludgeTwoBiogas} for both biogas volumes and composition and \texttt{sludgeTwoSetup} for grouping, substrate and inoculum masses, and headspace volumes.
...
In this example, we will use the long example data set included in the biogas package: \texttt{strawPressure} for headspace pressure in batch reactors, \texttt{strawComp} for methane content of biogas, and \texttt{strawSetup} for grouping, substrate and inoculum masses, and headspace volumes.

These data are interval-based BMP measurement of headspace pressure in 12 anaerobic reactors with straw as the substrate.
The experiment included 12 batch bottles on the following 6 variables: 
\begin{itemize}
...
As for the sludge data, cumulative production of biogas and CH$_4$ and production rates are essential values to estimate prior to evaluating BMP.
...
% Note warning: pres ranges from 101.3 kPa to 203.4356 kPa. Is this really correct?. This warning can be 'fixed' by substracting absolute from pres.resid or manually converting gauge pressure measurements to absolute. 
