...
Menometric methods require accumulation of biogas within the bottle headspace. Headspace pressure is typically measured with a portable manometer, which evaluates the pressure relative to atmospheric pressure. 

Determination of CH$_4$ volume from manometric measurements requires some data processing. 
...
If any CH4 measurements are missing, \texttt{``xCH4''} is interpolated by the external ``low-level'' function \texttt{interp()}. Here, the \texttt{imethod} argument can be used to define interpolation method (default is \texttt{``linear''}), which is passed to \texttt{interp()}.   
