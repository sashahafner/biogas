...
To go from data collected in the laboratory to biogas and methane (CH$_4$) production or biochemical methane potential (BMP), two high-level functions are needed: \texttt{cumBg()} (now replaced by \calcBg*()} functions) and \texttt{summBg()}.
