...
A newly developed software package for biogas calculations, known as the biogas package, is available for the R environment \cite{softwarex}. This package address issues with time-consuming calculations and lack of reproducible among laboratories for obtaining BMP.
