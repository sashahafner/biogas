...
Most of the arguments have default values, but to calculate CH$_4$ production we must provide values for at least  \texttt{dat} (data frame with respons variable. Here pressure measurements), \texttt{comp} (data frame with methane concentration), and \texttt{temp} (biogas temperature)\footnote{.      
