...
To calculate CH$_4$ production from these longcombo data, we must provide values for at least \texttt{dat} and \texttt{comp}, which is in a combined data frame (we will use \texttt{s3lcombo}), \texttt{temp} (biogas temperature), and \texttt{pres} (biogas pressure) along with the names of a few columns in our input data frame. Even though volume measurements and composition data are in a combined data frame, the \texttt{comp} argument has to be specified as \texttt{comp} to indicate composition data are provided.
We can use the default values \texttt{"longcombo"}, \texttt{"id"}, \texttt{"vol.ml"}, and \texttt{"xCH4"} for the \texttt{"data.struct"}, \texttt{"id.name"}, \texttt{"dat.name"}, and \texttt{"comp.name"} arguments, respectively. However, \texttt{"dat.name"} argument has to be provided to avoid error from selection of undefine columns.
