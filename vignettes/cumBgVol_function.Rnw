...
% Use \texttt{cumBg()} throughout. (Also for other function names)
...
% Would be good to refer to a section below on data structures. Otherwise this will be hard for new users to understand.
...
If separate data frames are used for \texttt{dat} and \texttt{comp}, the name must match. Time data may be POSIX objects, but then t0 will not be added to rows by the cumBg* functions. In addition the \texttt{addt0} argument is used to add row with "time zero" (\texttt{time.name = 0}) for each bottle in order to calculate production rates for the first observation (default = TRUE). Whereas, \texttt{showt0} determines if the "time zero" should be returned in the output (default = TRUE if time.name is numeric and contains 0 and otherwise FALSE). 

The \texttt{comp.name} argument is used to indicate which column within the \texttt{comp} data frame contains the CH$_4$ content (as mole fraction in dry biogas, normalized so the sum of mole fractions of CH$_4$ and CO$_2$ sum to unity) (default = xCH4). The \texttt{comp} argument may be a single value instead of a data frame or column. When providing a single value for \texttt{comp}, this value is applied to all observations, and the \texttt{comp.name} argument is not needed. Note that if the \texttt{comp} argument is not provided, cumBg* functions will return calculations on biogas only and no calculations on CH$_4$. 
xCH4 definition depends on cmethod argument descriped below. 
...
Input data may be structured in one of three ways: "longcombo", "long", and "wide." Default is "longcombo". All inputs are accepted, but the volumetric calculation methods within \texttt{cumBgVol()} only process "longcombo" data structure. "wide" and "long" data are restructured internally by the low-level function \texttt{cumBgDataPrep()}. In the following examples all three data structures will be addressed.
% Might add more details about each structure
...
In this example, we will use the "longcombo" example dataset included in the biogas package: \texttt{s3lcombo} for both biogas volumes and composition.
