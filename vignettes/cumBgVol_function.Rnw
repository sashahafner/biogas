...
%\VignetteIndexEntry{Calculating methane and biogas production and production rates from volumetric measurements}
...
\title{Calculating methane and biogas production and production rates from volumetric measurements}
...
The \texttt{comp.name} argument is used to indicate which column within the \texttt{comp} data frame contains the CH$_4$ content. Default is xCH$_4$. The \texttt{comp} argument may be a single value instead of a data frame or column. When providing a single value for \texttt{comp}, this value is applied to all observations, and the \texttt{comp.name} argument is not needed. Note that if the \texttt{comp} argument is not provided, \texttt{cumBgVol()} will return results for biogas only and not CH$_4$. The definition of xCH4 depends on the cmethod argument.
...
\label{structures}
Input data may be structured in one of three ways: "longcombo", "long", and "wide." Default is "longcombo", where the composition column is in the \texttt{dat} data frame and no separate composition data frame is required. In the following examples all three data structures will be addressed. For more details about each of the three data structure see \texttt{biogas\_quick\_start} vignette.
...
It is probably easier to understand the data in the output graphically. Plotting results is recommended, and can be used to catch strange responses.
...
\label{function_internals}
 which is internally passed to \texttt{cumBgDataPrep()} and restructured to \texttt{"longcombo"} structure prior to entering volumetric calculation methods.
