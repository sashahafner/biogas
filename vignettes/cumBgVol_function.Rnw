...
% Also may want to mention original cumBg() function.
...
% Could also mention that function accepts different data structures (but don't describe) in the overview
...
% Maybe this material should be in a section "Function internals" or something else. Most users will not need to look at it unless they are interested.
\begin{table}[h!]
  \begin{center}
...
Two methods are commonly used to evaluate volumetric biogas measurements. Method 1 is based on normalized CH$_4$ concentrations, whereas method 2 accounts for the actual CH$_4$ in the bottle headspace. Both methods are available through cumBgVol() and results is expected to be independent of method. The examples below describe cumulative biogas calculation from three datasets of different structures ("wide", "long", and "longcombo") using volumetric method 1. For method 2 examples, refer to \code{\link{cumBgMan}}. 
All external functions are within the biogas package


% I think the arguments should be in their own section
...
% OK. You could also manually enter the output here and use \verbatim{}.
% You can edit what I have pasted below (I wrapped the first few lines)
...
##     dat.name = "vol", comp.name = "xCH4", headspace = NULL, vol.hs.name = "vol.hs", 
##     cmethod = "removed", imethod = "linear", extrap = FALSE, 
##     addt0 = TRUE, showt0 = TRUE, dry = FALSE, empty.name = NULL, 
##     std.message = !quiet, check = TRUE, temp.std = getOption("temp.std", 
...
% Need to mention xCH4 definition depends on cmethod argument.
...
If any CH$_4$ measurements are missing, xCH4 is interpolated by the external "low-level" function \texttt{interp()}. Here, the \texttt{imethod} argument can be used to define interpolation method (default = "linear"), which is passed to interp().   
...
% Try to be consistent with active and passive voice ("we need to" vs. "is"). Either is fine.
...
% Maybe this description of data structures should go in a "data structures" section/sub-section. It seems out of place in an "examples" section.
...
% I don't think you need a subsubsection here (or in others below)
...
Furthermore, initial composition (xCH4) values are missing in the \texttt{s3lcombo} data frame. Therefore, we need to set \texttt{extrap = TRUE} for extrapolation of these initial CH$_4$ values
...
Additionally, the response variables are cumulative data and hence, we need to set \texttt{interval = FALSE}. 
...
In addition to interpolation for later observations, an extrapolation argument (\texttt{extrap}) can be provided if required. We do not have initial biogas composition (compare the heads of the \texttt{vol} and \texttt{comp} data frames) so we will need to extrapolate, in addition to interpolation. Therefore, we need to set \texttt{extrap = TRUE}.
