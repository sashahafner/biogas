...
Most of the arguments have default values, but to calculate CH$_4$ production we must provide values for at least  \texttt{dat} (data frame with response variable - here volume measurements), \texttt{comp} (data frame with methane concentration), and \texttt{temp} (biogas temperature)\footnote{.      
...
The \texttt{comp.name} argument is used to indicate which column within the \texttt{comp} data frame contains the CH$_4$ content (as mole fraction in dry biogas, normalized so the sum of mole fractions of CH$_4$ and CO$_2$ sum to unity) (default = xCH4). The \texttt{comp} argument may be a single value instead of a data frame or column. When providing a single value for \texttt{comp}, this value is applied to all observations, and the \texttt{comp.name} argument is not needed. Note that if the \texttt{comp} argument is not provided, \texttt{cumBgVol()} will return calculations on biogas only and no calculations on CH$_4$. The definition of xCH4 depends on the cmethod argument. 
...
If any CH$_4$ measurements are missing, xCH4 is interpolated by the external "low-level" function \texttt{interp()}. Here, the \texttt{imethod} argument can be used to define interpolation method (default = "linear"), which is passed to \texttt{interp()}.   
...
The data frame that is returned has maintained the "longcombo" structure with all the original columns in \texttt{s3lcombo} and contains additional columns from volumetric biogas calculations. 
...
To calculate CH$_4$ production from these "wide" structured data, we must provide values for at least \texttt{dat}, \texttt{comp}, \texttt{temp}, and \texttt{pres} along with the names of a few columns in our input data frame. The \texttt{dat} argument is set as the \texttt{feedVol} data frame, whereas \texttt{comp}, \texttt{temp}, and \texttt{pres} are set as single values of 1 (mole fraction), 0 (degree C), and 1 (atm), respectively. 
...
As mentioned, there are no ID columns for \texttt{data.struct = "wide"}. Instead column name of first column with response variables is used as ID code.  
...
Fixed data were made adding temperature and pressure to the \texttt{vol} data frame from the "long" structure example above. Temperature is set to a fixed value for all observations, whereas pressure is set to vary between observations using \texttt{rnorm()} having a mean value of 1 atm and a standard deviation of 0.001. It is possible to have temperature and pressure values that varies between measurements. If this was the case, the approach (function call) would be the same as shown below.    
<<>>=  
data("vol")
...
The arguments required to process these data using \texttt{cumBgVol()} are the same as in the "long" data structure example above, besides for the \texttt{temp} and \texttt{pres} arguments. Instead of fixed values, we now call a column name from \texttt{vol}. Therefore, we set \texttt{temp = "temp"} and \texttt{pres = "pres"}
<<>>=
...
Similar to cum.prod.l, the data frame that is returned has been restructured to "longcombo" structure and contains all the original columns in \texttt{vol}, plus additional columns from volumetric biogas calculations (ref. section "longcombo" data structure) 
...
By default, the unit of temperature and pressure values are set as degree celsius and atm, respectively. As stated above, these can be set using the \texttt{unit.temp} and \texttt{unit.pres} arguments. For the purpose of demonstrating the ussage of the two unit arguments, the already fixed pressure and temperature values in \texttt{vol} have been manipulated to be given in kelvin \texttt{K} and \texttt{kPa}, respectively.
<<>>=
vol$temp <- 35 + 273.15
vol$pres <- NA
...
Note the warnings about temperature and pressure range. This warning is a result of the default settings for pressure and temperature values. If we manually set the correct units, these warnings are avoided. 
<<>>=
cum.prod <- cumBgVol(vol, comp = comp, temp = "temp", pres = "pres",
                       data.struct = "long",
                       time.name = "days",
