...
% might be preferable to avoid including cumBg(), as these new functions should replace cumBg() at some point. 
% Instead descripe volumetric measurement methods shortly
% Sasha: "You do not need to include background on biogas production. But some background on volumetric methods, perhaps."
